...
// Relative /api works for both:
// - Local dev: Vite proxies /api → http://localhost:3001
// - Docker/prod: nginx proxies /api → http://backend:3001
...
api.interceptors.response.use(
  ...
  err => {
    if (err.response?.status === 401) {
      ['authToken','userId','userEmail','userName','userRole'].forEach(k => localStorage.removeItem(k));
...
export const adminApi = {
  ...
  scanSurvey:      (surveyId: string) => api.post(`/admin/anomalies/scan/${surveyId}`),
  flagAnomaly:     (id: string, flag: string) =>
                                         api.patch(`/admin/anomalies/${id}/flag`, { flag }),

  // Export
  exportCSV:   (id: string) => api.get(`/admin/export/surveys/${id}/csv`,   { responseType: 'blob' }),
  exportJSON:  (id: string) => api.get(`/admin/export/surveys/${id}/json`,  { responseType: 'blob' }),
  exportExcel: (id: string) => api.get(`/admin/export/surveys/${id}/excel`, { responseType: 'blob' }),
...
export const profileApi = {
  ...
  requestEmailChange: (newEmail: string)      => api.post('/profile/email/request', { newEmail }),
  confirmOldEmail:  (code: string)            => api.post('/profile/email/confirm-old', { code }),
  confirmNewEmail:  (code: string)            => api.post('/profile/email/confirm-new', { code }),
};

// ── Auth API helpers ────────────────────────────────────────────────────────

export const authApi = {
  requestForgotPassword: (email: string) => api.post('/auth/forgot-password/request', { email }),
...
export const friendsApi = {
  ...
  acceptFriendRequest: (friendId: string) => api.patch(`/friends/${friendId}/accept`),
  rejectFriendRequest: (friendId: string) => api.patch(`/friends/${friendId}/reject`),
