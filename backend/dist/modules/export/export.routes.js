...
const exportSchema = (format, summary, contentType) => ({
    tags: ['Export'],
    summary,
...
async function exportRoutes(fastify) {
    const exportService = new export_service_1.ExportService(fastify.prisma);
    // ── Shared access-check helper ─────────────────────────────────────────────
    async function checkOwner(req, reply) {
        const { id } = req.params;
     ...
     */
    function buildDisposition(title, ext) {
    ...
    fastify.get('/survey/:id/excel', {
        schema: exportSchema('Excel', 'Export survey results as Excel (.xlsx)', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'),
        preValidation: [fastify.authenticate],
    }, async (req, reply) => {
        try {
            const data = await checkOwner(req, reply);
    ...
    fastify.get('/survey/:id/docx', {
        schema: exportSchema('DOCX', 'Export survey results as Word document (.docx)', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'),
        preValidation: [fastify.authenticate],
    }, async (req, reply) => {
        try {
            const data = await checkOwner(req, reply);
